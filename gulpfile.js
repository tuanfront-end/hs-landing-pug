...
gulp.task("copyFonts", () => {
  return gulp
    ...
    .pipe(gulp.dest(config.react_public.fonts));
});
...
gulp.task("copyImagesDev", () => {
  return gulp
    .src(`${config.src.images}/**/*`)
    .pipe(gulp.dest(config.cache.images));
});

gulp.task("copyFontsDev", () => {
  return gulp
    .src(`${config.src.fonts}/**/*`)
    .pipe(gulp.dest(config.cache.fonts));
});

gulp.task("dev", ["serve", "watch", "copyImagesDev", "copyFontsDev"]);

gulp.task(
  "build",
  ["serve", "watch", "copyImages", "copyVendors", "copyHtml", "copyFonts"],
  () => {
    browserSync.exit();
    process.exit();
  }
